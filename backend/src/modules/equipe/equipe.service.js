...
async function listarEquipe(empresaId) {
  const { rows } = await query(
    `SELECT u.id, u.nome, u.email, u.telefone, u.ativo, u.papel_id, p.nome AS papel_nome
      ...
      WHERE u.empresa_id = $1 AND u.perfil IN ('central_admin','cliente') AND u.loja_id IS NULL
...
async function criarMembro({ empresaId, nome, email, telefone, senha, papelId, usuarioId, ip }) {
  ...
  let novo;
  try {
    novo = await authService.criarUsuario({ empresaId, perfil: 'central_admin', nome, email, telefone: telefone || null, senha, papelId: papelId || null });
  } catch (e) {
...
async function atualizarMembro({ empresaId, membroId, papelId, ativo, usuarioId, ip }) {
  const alvo = await query(`SELECT id FROM usuarios WHERE id = $1 AND empresa_id = $2 AND perfil IN ('central_admin','cliente') AND loja_id IS NULL`, [membroId, empresaId]);
