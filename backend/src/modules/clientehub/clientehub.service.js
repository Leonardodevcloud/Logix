...
async function obterRegras({ empresaId, lojaId }) {
  await exigirLoja(empresaId, lojaId);
  const { rows } = await query(`SELECT max_corridas_motoboy, raio_km FROM cliente_regras_acionamento WHERE loja_id = $1`, [lojaId]);
  if (rows[0]) return rows[0];
  // default (não persiste até salvar)
  return { max_corridas_motoboy: 3, raio_km: 5 };
}

async function salvarRegras({ empresaId, lojaId, maxCorridas, raioKm, usuarioId, ip }) {
  await exigirLoja(empresaId, lojaId);
  ...
  await query(
    `INSERT INTO cliente_regras_acionamento (loja_id, empresa_id, max_corridas_motoboy, raio_km, atualizado_em)
     VALUES ($1,$2,$3,$4, now())
     ON CONFLICT (loja_id) DO UPDATE SET max_corridas_motoboy = $3, raio_km = $4, atualizado_em = now()`,
    [lojaId, empresaId, max, raio]
  );
  registrarAuditoria({ empresaId, usuarioId, categoria: 'loja', acao: 'salvar_regras_acionamento', detalhe: { lojaId, max, raio }, ip }).catch(() => {});
  return { ok: true, max_corridas_motoboy: max, raio_km: raio };
}
