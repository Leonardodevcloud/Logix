...
export async function montar(container) {
  ...
  if (!document.getElementById('lx-rastreio-style')) {
    ...
    s.textContent = `
      ...
      .lx-mb-detail{position:absolute;top:14px;right:14px;bottom:14px;background:#fff;border-radius:12px;padding:0;z-index:400;width:340px;border:0.5px solid rgba(0,0,0,.1);display:none;flex-direction:column;overflow:hidden;box-shadow:0 10px 34px -12px rgba(4,44,83,.45)}
      ...
      .lx-detail-topo{padding:14px 16px}
      .lx-roteiro{flex:1;overflow-y:auto;padding:0 12px 12px;border-top:0.5px solid #eef2f7}
      .lx-roteiro-tit{font-size:10.5px;letter-spacing:.4px;text-transform:uppercase;color:#8AA2BE;font-weight:700;padding:12px 4px 6px}
      .lx-stop{display:flex;gap:10px;padding:8px 6px;border-radius:8px}
      .lx-stop:hover{background:#f6f9fc}
      .lx-stop.prox{background:#E4EEF9}
      .lx-stop-num{width:22px;height:22px;border-radius:50%;flex:none;display:grid;place-items:center;font-size:11px;font-weight:800;background:#fff;border:1.5px solid #e6edf5;color:#5c7189}
      .lx-stop.prox .lx-stop-num{background:#185FA5;border-color:#185FA5;color:#fff}
      .lx-stop-info{min-width:0;flex:1}
      .lx-stop-proto{font-size:12.5px;font-weight:800;color:#042C53}
      .lx-stop-dest{font-size:11px;color:#5c7189;line-height:1.35;overflow:hidden;text-overflow:ellipsis;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical}
  ...
  const btnRota = el('button', { class: 'lx-detail-rota', onClick: renderRotaSelecionado },
    el('span', { innerHTML: SVG.route }), 'Ver rota otimizada');

  const detalheRoteiro = el('div', { class: 'lx-roteiro' });
  const cardDetalhe = el('div', { class: 'lx-mb-detail' },
    el('div', { class: 'lx-detail-topo' }, detalheName, detalheSub, detalheRows, btnRota, detalheRotaInfo),
    detalheRoteiro);
  ...
  async function renderRotaSelecionado() {
    ...
    btnRota.childNodes[1].textContent = ' Otimizando…';
    limparRota();

    try {
      const r = await fetch(`${BASE}/motoboys/${m.id}/rota-otimizada`, {
    ...
    btnRota.childNodes[1].textContent = ' Ver rota otimizada';
  }
  ...
  function selecionarMotoboy(m) {
    ...
    // Roteiro (timeline) — ordem padrão até otimizar
    detalheRoteiro.innerHTML = '';
  ...
  function renderSidebar() {
    ...
    function mkCard(m, classe, i) {
      ...
      if (m.entregas_ativas > 0) {
        card.append(el('div', { class: 'lx-mb-entregas' },
          ico(SVG.pkg, '#FACC15'),
          `${m.entregas_ativas} entrega${m.entregas_ativas > 1 ? 's' : ''} ativa${m.entregas_ativas > 1 ? 's' : ''}`));
      }
      return card;
    }
