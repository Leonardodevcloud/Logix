...
// Campo com valor em fonte monoespaçada e botão copiar (usado nos tokens).
...
// Modal genérico. Recebe uma função "construir(fechar)" que devolve o conteúdo,
// assim o botão fechar já existe quando o conteúdo é montado.
function abrirModal(titulo, construir) {
  ...
  const fechar = () => { try { document.body.removeChild(overlay); } catch (e) {} };
  const bruto = typeof construir === 'function' ? construir(fechar) : construir;
  const filhos = Array.isArray(bruto) ? bruto : [bruto];
...
// Mostra os tokens gerados (uma única vez).
function mostrarTokens({ nome, cod_cliente, tokens }) {
  ...
  abrirModal('Tokens de autenticação' + (nome ? ' (' + nome + ')' : ''), [
    el('div', { style: 'background:rgba(220,38,38,.08);border:1px solid rgba(220,38,38,.3);border-radius:10px;padding:10px 12px;font-size:12.5px;color:var(--lx-erro,#dc2626);font-weight:600;margin-bottom:14px' },
      'Copie e guarde agora. Por segurança o segredo não é exibido novamente. Se perder, gere novos tokens.'),
    grid,
  ]);
}

// ── Modal de criar chave ─────────────────────────────────────────────────────
function modalNovaChave(lojas, aoSalvar) {
  const inpNome = el('input', { class: 'lx-input', placeholder: 'Ex.: ERP da loja' });
  const selLoja = el('select', { class: 'lx-input' }, el('option', { value: '' }, 'Selecione a loja...'));
  (lojas || []).forEach((l) => selLoja.append(el('option', { value: l.id }, l.nome_fantasia || l.nome || l.id)));
  const inpUrl = el('input', { class: 'lx-input', placeholder: 'https://sistema-da-loja.com/webhook (opcional)' });

  abrirModal('Nova integração', (fechar) => [
    el('div', { class: 'lx-field' }, el('label', {}, 'Nome da integração'), inpNome),
    el('div', { class: 'lx-field', style: 'margin-top:10px' }, el('label', {}, 'Loja'), selLoja),
    el('div', { style: 'font-size:11.5px;color:var(--lx-tinta-2);margin:-4px 0 10px' }, 'O Código do cliente é gerado para esta loja. Toda corrida enviada com ele nasce nesta loja e usa a tabela de preço configurada para ela.'),
    ...
    el('div', { style: 'display:flex;gap:8px;justify-content:flex-end' },
      ...
      el('button', { class: 'lx-btn lx-btn-primario', onClick: async () => {
        ...
        if (!selLoja.value) { toast('Selecione a loja', 'erro'); return; }
        try {
          const r = await post('/integracoes/chaves', {
            nome: inpNome.value.trim(), loja_id: selLoja.value, url_notificacao: inpUrl.value.trim() || null,
          });
          fechar();
...
// ── Card de uma chave ────────────────────────────────────────────────────────
function cardChave(c, recarregar) {
  const badge = el('span', {
  ...
  async function regenerar() {
    if (!confirm('Gerar novos tokens invalida os atuais. O sistema da loja precisará dos novos. Continuar?')) return;
  ...
  async function excluir() {
    if (!confirm('Excluir a integração "' + c.nome + '"? A loja perde o acesso imediatamente.')) return;
  ...
  return el('div', { class: 'lx-card', style: 'padding:18px 20px;margin-bottom:16px;display:flex;align-items:center;gap:16px;flex-wrap:wrap' },
    ...
    el('div', { style: 'flex:1;min-width:200px' },
      ...
      el('div', { style: 'font-size:12.5px;color:var(--lx-tinta-2);margin-top:2px' },
        'Guia completo: endpoints, exemplos de sucesso e erro, webhook e lista de erros. Pronto para repassar ao time da loja.'),
...
export async function montar(container) {
  ...
  async function recarregar() {
    ...
    if (!chaves.length) {
      lista.append(el('div', { class: 'lx-card', style: 'padding:28px;text-align:center;color:var(--lx-tinta-2)' },
        ...
        'Crie uma chave para a loja enviar corridas para a plataforma.'));
      return;
    }
    chaves.forEach((c) => lista.append(cardChave(c, recarregar)));
  }

  try { lojas = await get('/lojas'); } catch (e) { lojas = []; }
  ...
  container.append(casca('Integrações (API)', conteudo,
    'Chaves de API para a loja criar e acompanhar corridas na plataforma.'));
  await recarregar();
}
