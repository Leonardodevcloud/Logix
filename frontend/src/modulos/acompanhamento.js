...
// Copia o link PÚBLICO de rastreio da entrega para enviar ao cliente final.
function copiarLinkRastreio(c) {
...
function toastRastreio(msg, erro) {
  ...
  t.textContent = msg;
  t.style.cssText = 'position:fixed;bottom:26px;left:50%;transform:translateX(-50%);background:' + (erro ? '#D0584F' : '#042C53') + ';color:#fff;font-size:13px;font-weight:700;padding:11px 18px;border-radius:10px;box-shadow:0 8px 24px rgba(0,0,0,.3);z-index:99999;opacity:0;transition:opacity .2s';
...
// [acoes-linha v6] trava o layout das ações em uma linha só, vencendo qualquer
...
try { console.log('[acompanhamento] acoes-linha v6 ativo'); } catch (_) {}
...
export async function montar(container) {
  ...
  let _aba = 'sem';
  let _vista = (typeof localStorage !== 'undefined' && localStorage.getItem('lx_acomp_vista') === 'kanban') ? 'kanban' : 'lista';
  let _busca = '';
  ...
  let _campo = '';
  const selCampo = el('select', { class: 'lx-input', style: 'height:34px;line-height:1.4;padding-top:0;padding-bottom:0;flex:none;width:145px' },
  ...
  const estiloVistaBtn = (ativo) => `border:0;background:${ativo ? 'var(--lx-azul-primario)' : 'transparent'};color:${ativo ? '#fff' : 'var(--lx-tinta-2)'};font:inherit;font-size:13px;font-weight:600;padding:6px 12px;border-radius:7px;cursor:pointer;display:inline-flex;align-items:center;gap:6px`;
  const btnVLista = el('button', { onClick: () => setVista('lista') }, svgIcone(P.lista, 15), el('span', {}, 'Lista'));
  const btnVKanban = el('button', { onClick: () => setVista('kanban') }, svgIcone(P.kanban, 15), el('span', {}, 'Kanban'));
  ...
  aplicarToggle();
  const barraTopo = el('div', { style: 'display:flex;gap:8px;align-items:center;margin-bottom:12px' }, selCampo, buscaWrap, btnFiltros, el('div', { style: 'flex:1' }), toggleVista, el('span', { title: 'build acoes-linha', style: 'font-size:10px;color:var(--lx-tinta-3);align-self:center;margin-left:2px' }, 'v6'));
  ...
  const kanbanWrap = el('div', { style: 'display:none' });
  const conteudo = el('div', {}, barraTopo, painel, avisoEl, abas, barraSel, tabelaWrap, kanbanWrap);
